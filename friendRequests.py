...
class Friends(object):
    ...
    def isFriends(self, current_user, profile_user):
        ...
        if str(current_user['_id']) in profile_user['friends'] or\
                        str(profile_user['_id']) in current_user['friends']:
            print '-----------yes freinds--------------'
            return False
        print '----------------not friends--------------'
        return True
...
class MatchUnmatch(object):
        ...
        post_status = post.update( {'_id': ObjectId(self.postid)},
                                     {
                                         "$push" :{
                                            "interestedPeople": {
                                                'interested_person': self.cuserid,
    ...
    def unMatch(self):
        ...
        user_status = account.update( {'_id': ObjectId(self.authorid),
                                       "notifications.friendid" : self.cuserid,
                                       "notifications.postid" : self.postid,
        ...
        post_status = post.update( {'_id': ObjectId(self.postid),
                                    'interestedPeople.interested_person': self.cuserid,
                                    },
                                     {
                                         "$pull" :{
                                            "interestedPeople": {
